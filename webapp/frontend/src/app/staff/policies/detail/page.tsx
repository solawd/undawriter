...
function StaffPolicyDetailContent() {
  ...
  useEffect(() => {
    if (id) {
      fetchWithAuth(`/api/v1/staff/policies/${id}`)
        .then(res => res.json())
        .then(data => {
          setData(data);
          setLoading(false);
  ...
  if (!data || !data.policy) return <div className="p-8 text-center text-red-500">Policy not found.</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      <div className="flex justify-between items-center print:hidden">
        <h1 className="text-3xl font-bold text-primary">Policy POL-{policy.id}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6 print:w-full">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 print:block">
            <h2 className="text-xl font-bold text-gray-800 mb-4 border-b pb-2">Customer Details</h2>
            <div className="grid grid-cols-2 gap-y-4 text-sm">
                <p className="text-gray-500">Full Name</p>
                <p className="font-semibold text-right">{policy.user?.fullName}</p>
                <p className="text-gray-500">Phone</p>
                <p className="font-semibold text-right">{policy.user?.phoneNumber}</p>
                <p className="text-gray-500">Ghana Card</p>
                <p className="font-semibold text-right">{policy.user?.ghanaCardId}</p>
            </div>
          </div>
            ...
            <div className="bg-white border-2 border-black p-1 rounded-sm shadow-lg relative print:shadow-none font-sans relative overflow-hidden">
              <div className="border border-black w-full h-full p-4 relative pb-6 z-10">
                ...
                <div className="flex justify-between items-center mb-6">
                  <div className="space-y-1 text-[13px] text-black w-full">
                    <div className="flex">
...
export default function StaffPolicyDetail() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-gray-500">Loading policy context...</div>}>
      <StaffPolicyDetailContent />
    </Suspense>
  );
}
